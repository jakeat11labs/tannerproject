...
| User intent | Load |
| --- | --- |
| Set up the project, generate audio, debug a generation issue, pick voices, write expressive lines, troubleshoot anything operational | `.claude/skills/interview-audio/SKILL.md` |
...
If both could apply, default to `interview-audio` for "how do I do X with this project" and `tutor-mode` for "how does X work." When in doubt, ask the user.
...
- **`dialogue.js`** — dialogue mode via `/v1/text-to-dialogue`. Default model `eleven_v3` (the documented path); `eleven_v4` and `eleven_v4_hq` also work despite the docs claiming v3-only — confirmed via live API testing. Honors `ELEVEN_MODEL_ID`. Chunk size is model-aware: v3 → ≤1900 chars, v4/v4_hq → ≤1400 chars (their endpoint cap is 1500).
...
- **Default model**: `eleven_v4` for segment mode, `eleven_v3` for dialogue mode (the documented path). `ELEVEN_MODEL_ID` overrides both. v4/v4_hq are technically usable in dialogue mode too (live API testing confirms), but v4 dialogue tends to read fast — useful to know it's an option, not the default. See `.claude/skills/interview-audio/references/modes.md` for the full tradeoff table.
...
- `samples/` — pre-rendered MP3s (A: dialogue+v3, B: segment+v3, C: segment+v4 *(default)*, D: dialogue+v4 *(experimental, fast)*)
...
- `.claude/skills/interview-audio/` — operational walkthrough skill
...
Tanner is new to development and Claude Code. Lean toward warmth, plain language, and one-step-at-a-time guidance — see `.claude/skills/interview-audio/SKILL.md` for the operational walkthrough script and `.claude/skills/tutor-mode/SKILL.md` for tutor-mode behavior.
